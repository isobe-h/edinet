...
class FinancialDataProcessor:
    ...
    def _get_item_name(self, item_name, term=None) -> str:
        if term:
            values = self.df.loc[
                (self.df["項目名"] == item_name)
                & (self.df["相対年度"].str.contains(term)),
                "項目名",
            ].values
            if len(values) > 0:
                return values[0]
        ...
        if term:
            ...
            for category in ["連結", "個別"]:
                match = self.df.loc[
                    query_base
                    & query_term
                    & (self.df["連結・個別"].str.contains(category)),
                    "値",
                ]
                if not match.empty:
                    return match.iloc[0]
            ...
            other_match = self.df.loc[
                query_base
                & query_term
                & (~self.df["連結・個別"].str.contains("連結|個別", regex=True)),
                "値",
            ]
    ...
    def calculate_financial_metrics(self):
        ...
        operating_profits = self._get_float_values_by_name(
            "営業利益又は営業損失（△）", "前期", "当期"
        )
    ...
    def get_report(self):
        ...
        consolidated_debt_detail_title = (
            "借入金等明細表、連結財務諸表 [テキストブロック]"
        )
        ...
        debt_detail = self._get_first_value_by_name(
            consolidated_debt_detail_title, "当期"
        )
        if debt_detail == "":
            debt_detail = self._get_first_value_by_name(debt_detail_title, "当期")
        debt_rate = self._calculate_weighted_average_cost(debt_detail)
        ...
        number_of_stock = self._get_float_values_by_name(
            "発行済株式総数（普通株式）", "当期末"
        )[0]
        number_of_company_stock = self._get_float_values_by_name(
            "自己名義所有株式数（株）、自己株式等", "当期末"
        )
        fcf = (
        ...
        effective_tax_rates = [
            self._calculate_effective_tax_rate(
                self._get_float_values_by_name("法人税等", "前期")[0],
                self._get_float_values_by_name(
                    "税引前当期純利益又は税引前当期純損失（△）", "前期"
                )[0],
            ),
            self._calculate_effective_tax_rate(
                self._get_float_values_by_name("法人税等", "当期")[0],
                self._get_float_values_by_name(
                    "税引前当期純利益又は税引前当期純損失（△）", "当期"
                )[0],
            ),
        ]
        noplats = [
        ...
        invested_capitals = self._calculate_invested_capital(
        ...
        tangible_fixed_assets = self._get_float_values_by_name(
            "有形固定資産", "前期", "当期"
        )
        # 累計減価償却額
        ...
        return {
            ...
            **idle_assets,
            ...
            "NOPLAT": noplats,
            "ROIC(NOPLAT/投下資本)": self._calculate_roic(noplats, invested_capitals),
            "ROIC(NOPAT/投下資本)": self._calculate_roic(
    ...
    def _calculate_weighted_average_cost(self, text_data):
        # 該当事項がない場合のチェック
        ...
        weighted_average_cost = (
            total_weighted_rate / total_balance if total_balance else 0
        )
        return weighted_average_cost

    # 実効税率を取得
    def _calculate_effective_tax_rate(
        self, tax: float, net_income_before_tax_current_period: float
    ):
        # 実効税率
    ...
    def _calculate_roic(
        ...
        return [
            x
            for x in map(lambda x, y: round(x / y, 4) * 100, incomes, invested_capital)
        ]
    ...
    def get_net_operating_capital(self) -> NetOperatingCapital:
        ...
        inventories_item_names = set(
            [
                self._get_item_name(x, "当期")
                for x in self.inventories_items
                if self._get_item_name(x, "当期") != ""
            ]
        )
        purchase_debt_item_names = set(
            [
                self._get_item_name(x, "当期")
                for x in self.purchase_debt_items
                if self._get_item_name(x, "当期") != ""
            ]
        )
        sales_receivables = {
        ...
        sums = [
            ...
            + sum_of_sales_receivables[0]
            - sum_of_purchase_debt[0],
            sum_of_inventories[1]
            + sum_of_sales_receivables[1]
            - sum_of_purchase_debt[1],
        ]
    ...
    def get_idle_assets(self):
        return {
            "投資有価証券": self._get_float_values_by_name(
                "投資有価証券", "前期", "当期"
            ),
            "現金及び預金": self._get_float_values_by_name(
                "現金及び預金", "前期", "当期"
            ),
        }
    ...
    def get_financial_summary(self) -> FinancialSummary:
        ...
        cost_of_sales = [
            x - y
            for x, y in zip(
                revenues, self._get_float_values_by_name("売上原価", "前期", "当期")
            )
        ]
        # 一般管理費及び販売費
        ...
        # 潜在株式調整後１株当たり当期純利益
        net_income_per_share_adjusted_for_potential_stock: list[float] = (
            self._get_float_values_by_name(
        ...
        return {
            ...
            "capital_expenditure": self._get_float_values_by_name(
                "設備投資額、設備投資等の概要", "前期", "当期"
            ),
